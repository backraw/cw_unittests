  ...
  switch (DBGMCU->IDCODE & 0xFFF)
    {
      ...
      case 0x434: // STM32F469/STM32F479
        // Fixme add support for DB1M=1 option bit i.e. two banks
        ramsize = (48*1024);
        geometry = geometry2;
        FLASH_SIZE = (1024*1024*2);
        break;
...
#if 0
  {
    uint8_t *erase_start=0x90000000;
    size_t erase_size;    
    static unsigned char buffer[1024];
    for (int i=0;i<sizeof(buffer);i++)
      buffer[i] = i+1;
    int res = libmem_erase(erase_start, sizeof(buffer), &erase_start, &erase_size);
    res = libmem_write(0x90000000, buffer, sizeof(buffer));    
    res = libmem_flush();
    memset(buffer, 0x00, sizeof(buffer));
    res = libmem_read(buffer, 0x90000000, sizeof(buffer));
