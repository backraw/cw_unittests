...
STM32SimulatorMemoryImpl::STM32SimulatorMemoryImpl() :
  flash(0), sram(0), peripherals(0), scs(0), eppb(0), fsmc(0), dataram(0)
{
  int i;
  for (i=0;i<4;i++)
    fsmc_nor_psram[i] = 0;
...
STM32SimulatorMemoryImpl::setSpecification(bool le, unsigned argc, const char *argv[])
{
  if (argc < 3 || argc > 7)
    return false;
  if (strstr(argv[0], "STM32F0")==argv[0] || strstr(argv[0], "STM32F1")==argv[0])
    {
      peripherals = new STM32F10xPeripheralRegion();
    }
  else if (strstr(argv[0], "STM32F2")==argv[0] || strstr(argv[0], "STM32F4")==argv[0] || strstr(argv[0], "STM32F7")==argv[0])
    {
  ...
  else if (strstr(argv[0], "STM32L4")==argv[0])
    {
