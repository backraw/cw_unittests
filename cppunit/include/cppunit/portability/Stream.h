...
{
public:
   FileStreamBuffer( DEBUG_FILE *file )
      : file_( file )
   {
   }

   DEBUG_FILE *file() const
   {
      return file_;
   }
...
public: // overridden from StreamBuffer
   ...
   {
      if ( file_ )
         debug_fwrite( text, sizeof(char), length, file_ );
   }
...
{
   static OFileStream stream( DEBUG_STDOUT );
   return stream;
}

inline OStream &stdCErr() 
{
   static OFileStream stream( DEBUG_STDERR );
   return stream;
}
